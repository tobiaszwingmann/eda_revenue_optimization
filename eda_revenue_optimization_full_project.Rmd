...
ctr_perform_df %>% 
  ggplot(aes(x=logViews)) + 
  geom_histogram()

ctr_perform_df %>% 
  ggplot(aes(x=logKlicks)) + 
  geom_histogram()

ctr_perform_df %>% 
  ggplot(aes(y=logViews, x=logKlicks)) +
  geom_point()
>>>>>>> 608520e31e123eafacf79334554eee969a86fedb
...
summary(clicks_lm_1) # 0.699
plot(clicks_lm_1, which=1:4)

shapiro.test(clicks_lm_1$residuals) #.89552
...
=======
clicks_lm_2 <- lm(logKlicks ~ logViews, data=ctr_perform_df[-ctr_perform_outliers,])
summary(clicks_lm_2) #.83 r-squared
